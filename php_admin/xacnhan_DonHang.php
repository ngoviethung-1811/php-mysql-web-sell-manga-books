...
<?php
    ...
    function xacNhanDonHang($maHD, $ngayGiao) {
        global $conn;

        $result = mysqli_query($conn, "UPDATE hoadon
            SET tinhTrang = 1, ngayGiao = '$ngayGiao'
            WHERE maHD = '$maHD'");
    ...
    if (isset($_GET['maHD'])) {
        ...
        <input type='submit' value='Xác nhận' name='xacnhan' id='btnXacNhan'/>";

        if (isset($_POST['xacnhan'])) {
            $kqXN = xacNhanDonHang($maHD, $ngayGiao);
            
            if ($kqXN) {
                $thongbao = "<p align=center><font color=green>Xác nhận đơn hàng thành công!</font></p>";
                $buttons = "<input type='button' value='Huỷ' name='huy' id='btnHuy' class='disable' onclick='history.back()'/>
                    <input type='submit' value='Xác nhận' name='xacnhan' id='btnXacNhan' class='disable'/>";
                $txtDisabled = 'disabled';
            }
            else $thongbao = "<p align=center><font color=red>Xác nhận đơn hàng không thành công!</font></p>";
        }
...
<link rel="stylesheet" href="../css/xacnhan_DonHang.css">

<main>
<p id='pageCaption'>XÁC NHẬN ĐƠN HÀNG</p>
<form action="" method="post">
    <table>
        ...
        <tr>
            <td>Ngày giao:</td>
            <td><input type="date" <?php echo $txtDisabled; ?> id="inputNgayGiao" name="ngayGiao" value="<?php echo $ngayGiao; ?>"></td>
        </tr>
        <tr>
            <td>Tình trạng:</td>
            <td>
