...
<?php
    ...
    if (isset($_GET['maTG'])) {
        ...
        $tacgia_query = "SELECT tenTG FROM tacgia WHERE maTG = '$maTG'";
        $tacgia_result = mysqli_query($conn, $tacgia_query);
        $tacgia_row = mysqli_fetch_assoc($tacgia_result);
        $tentg = $tacgia_row['tenTG'];
 
        echo '<div align=center><h3>Truyện của ' . $tentg . '</h3></div>'; 
