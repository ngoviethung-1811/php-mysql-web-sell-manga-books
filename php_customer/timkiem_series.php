...
<?php
    require("connect.php");
    if (isset($_GET['maSeries'])) {
        $maSeries = $_GET['maSeries'];

        $series_query = "SELECT tenSeries FROM series WHERE maSeries = '$maSeries'";
        $series_result = mysqli_query($conn, $series_query);
        $series_row = mysqli_fetch_assoc($series_result);
        $tenseries = $series_row['tenSeries'];

        echo '<div align=center><h3>Truyện thuộc series ' . $tenseries . '</h3></div>';   
            ...
            WHERE truyen.maSeries = '$maSeries'
