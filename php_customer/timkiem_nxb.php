...
<?php
    ...
    if (isset($_GET['maNXB'])) {
        ...
        $nxb_query = "SELECT tenNXB FROM nhaxuatban WHERE maNXB = '$maNXB'";
        $nxb_result = mysqli_query($conn, $nxb_query);
        $nxb_row = mysqli_fetch_assoc($nxb_result);
        $tennxb = $nxb_row['tenNXB'];

        echo '<div align=center><h3>Truyện của ' . $tennxb . '</h3></div>';
        ...
        if (mysqli_num_rows($result) > 0) {
            ...
            echo "Không có truyện nào thuộc Thể loại này.";
        }
    } else {
